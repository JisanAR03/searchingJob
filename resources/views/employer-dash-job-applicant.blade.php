      ...
      <div class=" p-3 flex flex-col space-y-3">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center w-full rounded-xl shadow-custom px-9 md:px-2 xl:px-9 py-5 space-y-3 md:space-y-0">
          <div class="flex flex-col md:flex-row justify-center items-center gap-1 ">
            <div class="w-full xl:w-56 py-1">
              <form action="{{route('job_seeker')}}" method="GET">
                <input type="hidden" name="id" value="{{$data->JobSeekerID}}">
                <button><p class="text-xs font-medium">{{$data->seeker_data->FullName}}</p></button></form>
              <p class="text-[10px] leading-4 font-medium text-button">Skill :  {{$data->seeker_data->skill_name}}</p>
              <p class="text-[10px] text-bodyText">Applied date : {{ date('F d, Y', strtotime($data->created_at)) }}</p>
            </div>
            <div class="flex justify-between fCFMV fBHFMV fWIMV">
              <div class="w-32 md:w-28 xl:w-32 space-y-3 px-0 xl:px-3.5 fWIMV">
                ...
                <p class="text-xs text-button dIBFMV">{{$data->seeker_data->experience}}</p>
              </div>
              <div class="w-32 md:w-28 xl:w-32 md:border-y-0 md:border-x border-[#A7A7A7] space-y-3 px-1 xl:px-3.5 fWIMV">
                ...
                @if($data->seeker_data->AppExperienceReview == 1)
                <p class="text-xs text-button dIBFMV">Yes</p>
                @else
                <p class="text-xs text-button dIBFMV">No</p>
                @endif
              </div>
              <div class="w-32 md:w-28 xl:w-32 md:border-y-0 md:border-x border-[#A7A7A7] space-y-3 px-1 xl:px-3.5 fWIMV">
                <p class="text-xs text-bodyText dIBFMV">Expected Annual Salary </p>
                <p class="text-xs text-button dIBFMV">{{$data->seeker_data->SalaryExpectation}}</p>
              </div>
            </div>
          </div>
          <div class="flex justify-center items-center text-center gap-3">
            ...
            <form action="{{route('delete-job-applicant')}}" method="POST">@csrf
            <input type="hidden" name="id" value="{{$data->JobApplicationID}}">
            <button>
            <svg   fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 bg-primary rounded p-1 stroke-2 text-button">
        ...
        <div class="pagination mt-5">
          ...
          @else
              ...
              <a href="{{ url('job-applicants?page=' . $application_data->currentPage() - 1) }}">&laquo;</a>
          @endif
          ...
          @for ($i = $start; $i <= $end; $i++)
              ...
              @else
                  <a href="{{ url('job-applicants?page=' . $i) }}">{{ $i }}</a>
              @endif
          @endfor
          ...
          @if ($application_data->hasMorePages())
              <a href="{{ url('job-applicants?page=' . $application_data->currentPage() + 1) }}">&raquo;</a>
          @else
