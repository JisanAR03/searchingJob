    ...
    <div class="w-full md:w-3/4 space-y-10 ">
      <form action="{{route('job-seeker-account-data-up')}}" method="POST" class="space-y-8  py-12 px-4 md:px-8 lg:px-16 shadow-custom">@csrf
        ...
        <div class="flex flex-col md:flex-row justify-around items-start md:items-center w-full">
          ...
          <input type="text" class="input-class w-full  md:w-[500px]" name="seekerName" value="{{$user_data->FullName}}" placeholder="Jubayer Islam">
        </div>
        <div class="flex flex-col md:flex-row justify-around items-start md:items-center w-full ">
          <label for="" class="w-60 font-medium">Change Email</label>
          <input type="email" class="input-class w-full  md:w-[500px]" name="new_email" placeholder="abc 12345@gmail.com">
        </div>
        <div class="flex flex-col md:flex-row justify-around items-start md:items-center w-full ">
          <label for="" class="w-60 font-medium">Confirm Email address</label>
          <input type="email" class="input-class w-full  md:w-[500px]" name="new_email_conf" placeholder="abc 12345@gmail.com">
        </div>
        <div class="flex flex-col md:flex-row justify-around items-start md:items-center w-full">
